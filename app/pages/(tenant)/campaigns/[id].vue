...
const clearSchedule = async () => { if (!campaign.value) return; actionLoading.value = true; try { await $api.patch(`/campaigns/${campaign.value.id}`, { clearSchedule: true }); await load() } catch (cause) { error.value = normalizeApiError(cause, 'Schedule could not be cancelled.') } finally { actionLoading.value = false } }
...
<template>
  <section class="campaign-detail-page">
    ...
    <div v-if="scheduleOpen" class="campaign-modal-backdrop" @click.self="scheduleOpen = false"><section class="campaign-confirm-modal" role="dialog" aria-modal="true"><h2>Schedule auto-send</h2><p>The campaign will be sent automatically at the selected time.</p><label>Send at (local time)<input v-model="scheduledAt" type="datetime-local" /></label><footer><button type="button" class="campaign-secondary-button" @click="scheduleOpen = false">Cancel</button><button type="button" class="campaign-primary-button" :disabled="actionLoading || !scheduledAt" @click="schedule">{{ actionLoading ? 'Saving…' : 'Enable auto-send' }}</button></footer></section></div>
  </section>
</template>
